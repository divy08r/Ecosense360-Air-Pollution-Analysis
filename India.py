...
st.set_page_config(
  ...
  page_icon=
  'https://cdn3d.iconscout.com/3d/premium/thumb/green-earth-6855145-5625018.png?f=webp'
)
...
st.subheader('PM2.5')
st.write(
  'PM2.5, or Particulate Matter 2.5, is a critical air quality parameter that refers to tiny airborne particles with a diameter of 2.5 micrometers or less. These particles are so small that they can easily penetrate deep into the lungs and even enter the bloodstream when inhaled. PM2.5 is a significant concern in air pollution analysis as it includes various harmful substances like dust, smoke, and pollutants.'
)

pm2_5fig = support.pm2_5
st.pyplot(pm2_5fig)
st.markdown(
  "<i><h6 style='text-align: center; color: white; margin-top: -2rem;'>City v/s PM2.5 plot</h6></i>",
  unsafe_allow_html=True)
...
st.subheader('PM10')
st.write(
  'PM10, or Particulate Matter 10, represents another crucial parameter in air quality analysis. These microscopic particles, with a diameter of 10 micrometers or less, are found in the air we breathe and can have a significant impact on our health. PM10 includes a range of materials, from dust and pollen to soot and allergens.'
)

pm10fig = support.pm10
st.pyplot(pm10fig)
st.markdown(
  "<i><h6 style='text-align: center; color: white; margin-top: -2rem; background: white'>City v/s PM10 plot</h6></i>",
  unsafe_allow_html=True)
st.image(
...
st.markdown(
  "<h6 style='text-align: center; color: grey; margin-top: -2rem; background: white'>PM2.5 vs. PM10: The Difference in Particle Air Pollution</h6>",
  unsafe_allow_html=True)
...
st.pyplot(india)
st.markdown(
  "<div style='text-align: center; color: grey; background: white'>Air quality in India exhibits substantial variation across states. For instance, states like Himachal Pradesh and Kerala typically enjoy better air quality owing to their cleaner environments and lower industrialization. In contrast, states such as Delhi and Uttar Pradesh often grapple with severe pollution issues due to urbanization and industrial emissions. Each state adopts specific measures to address its unique air quality challenges, reflecting the diverse efforts made to safeguard public health.</div>",
  unsafe_allow_html=True)

st.subheader('AQI Bucket')
st.write(
  'AQI (Air Quality Index) buckets categorize air quality into different ranges, such as "Good," "Moderate," "Unhealthy," and "Hazardous," to help people understand the level of pollution in their area easily.'
)
st.image(
  'https://www.indiatogether.org/uploads/picture/image/2590/IT_airquality.png')
st.sidebar.image('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRmjF9Z-Ylbz8DdpjAe2oI3R3sONeUcrfOSBQ&usqp=CAU')
...
st.plotly_chart(support.airQuality, use_container_width=True)
...
st.plotly_chart(aqStation, use_container_width=True)
...
st.markdown('''
    ...
    <div style="width: 100%; border-radius: 0; background-image: url('https://files.123freevectors.com/wp-content/original/161337-lime-green-texture-background.jpg'); background-size: cover; display: flex;flex-direction: column; justify-content: center; align-items: center; flex-wrap: wrap; padding: 2rem;">
            <img style="width: 50%; margin-bottom: 1rem; border-radius: 0;" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRmjF9Z-Ylbz8DdpjAe2oI3R3sONeUcrfOSBQ&usqp=CAU" alt="">
